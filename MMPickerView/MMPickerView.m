...
-(void)initializePickerViewInView: (UIView *)view
    ...
    if (chosenObject != nil) {
        NSArray *indexArray = [self indexPathForChosenObject:chosenObject];
        selectedRow = [indexArray[0] integerValue];
        selectedComponent = [indexArray[1] integerValue];
    }else{
        selectedRow = 0;
        selectedComponent = 0;
    }
    ...
    //Set selected row
    [_pickerView selectRow:selectedRow inComponent:selectedComponent animated:YES];
}

- (NSArray*) indexPathForChosenObject:(id)chosenObject {
    
    NSNumber* row;
    NSNumber* column;
    
    for (NSArray* objectArray in _pickerViewArrayOfArrays) {
        @try {
            
            row = [NSNumber numberWithInteger:[objectArray indexOfObject:chosenObject]];
            column = [NSNumber numberWithInteger:[_pickerViewArrayOfArrays indexOfObject:objectArray]];
